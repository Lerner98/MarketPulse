...
export const BurnRateGauge = ({ data, segmentType, isLoading = false }: BurnRateGaugeProps) => {
  ...
  const chartData = {
    labels: data.map(item => item.segment_value),
    datasets: [
      {
        label: 'יחס הוצאה',
        data: data.map(item => item.burn_rate_pct),
        backgroundColor: data.map(item => getColor(item.burn_rate_pct)),
        borderColor: data.map(item => getBorderColor(item.burn_rate_pct)),
        borderWidth: 2
      }
    ]
  };
